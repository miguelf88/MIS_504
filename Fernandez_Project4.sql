...
-- 5. We need a report that tells us everything we need to place an order. This should be only non-discontinued products whose (unitsInstock + unitsOnOrder) is less than or equal to the Reorderlevel.
-- As the final column, it should show how many to order. We usually order enough so that (unitsInStock+UnitsOnOrder) is equal to twice the reorderlevel. Columns should be SupplierID, CompanyName, companyphone,
-- productID, productName, amount to order. Order by SupplierID, then by productID.
